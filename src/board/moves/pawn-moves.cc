...
namespace board
{
                                              ...
                                              Bitboard double_moves_bitboard, Bitboard *piece_board)
    {
        ...
        while (_BitScanForward64(&index, single_moves_bitboard))
        {
            ...
            if (validate_move(piece_board, nullptr, position, move))
            {
                ...
                if (move & RANK1OR8)
                {
                    moves.push_back({ move, piece_board, nullptr, _en_passant, white_turn, MoveType::PROMOTION_QUEEN });
                    moves.push_back(
                        { move, piece_board, nullptr, _en_passant, white_turn, MoveType::PROMOTION_KNIGHT });
                    moves.push_back({ move, piece_board, nullptr, _en_passant, white_turn, MoveType::PROMOTION_ROOK });
                    moves.push_back(
                        { move, piece_board, nullptr, _en_passant, white_turn, MoveType::PROMOTION_BISHOP });
                }
                else
                {
                    moves.push_back({ move, piece_board, nullptr, _en_passant, white_turn, MoveType::NONE });
                }
            }
        }
        ...
        while (_BitScanForward64(&index, double_moves_bitboard))
        {
            ...
            if (validate_move(piece_board, nullptr, position, move))
                moves.push_back({ position | move, piece_board, nullptr, _en_passant, white_turn, type });
                                                ...
                                                Bitboard *piece_board, bool east_attacks)
    {
        ...
        while (_BitScanForward64(&index, moves_bitboard))
        {
            ...
            if (move & _en_passant)
            {
                Bitboard enemy_pawn_position = white_turn ? move >> 8 : move << 8;
                Bitboard enemy_pawn_offset = move | enemy_pawn_position;
                Bitboard *enemy = get_board_at_position(enemy_pawn_position, !white_turn);
                Bitboard *enemies = white_turn ? &_blacks : &_whites;
                ...
                *enemies ^= enemy_pawn_offset;
                ...
                if (validate_move(piece_board, enemy, position, move))
                    moves.push_back(
                        { position | move, piece_board, enemy, _en_passant, white_turn, MoveType::EN_PASSANT });
            ...
            else
            {
                ...
                if (validate_move(piece_board, enemy, position, move))
                {
                    if (move & RANK1OR8)
                    {
                        ...
                        moves.push_back(
                            { full_move, piece_board, enemy, _en_passant, white_turn, MoveType::PROMOTION_QUEEN });
                        moves.push_back(
                            { full_move, piece_board, enemy, _en_passant, white_turn, MoveType::PROMOTION_KNIGHT });
                        moves.push_back(
                            { full_move, piece_board, enemy, _en_passant, white_turn, MoveType::PROMOTION_ROOK });
                        moves.push_back(
                            { full_move, piece_board, enemy, _en_passant, white_turn, MoveType::PROMOTION_BISHOP });
                    }
                    else
                    {
                        moves.push_back(
                            { position | move, piece_board, enemy, _en_passant, white_turn, MoveType::NONE });
                    }
                }
            }
